...
map <int, int> Map;
int plate[3][9][9];
int anextx[8] = { 1,1,-1,-1,2,2,-2,-2 };
int anexty[8] = { 2,-2,2,-2,1,-1,1,-1 };
int bnextx[8] = { 0,0,1,-1,1,1,-1,-1 };
int bnexty[8] = { 1,-1,0,0,1,-1,1,-1 };
int cnextx[6] = { 1,1,1,-1,-1,-1 };
int cnexty[6] = { 1,0,-1,1,0,-1 };
//K 1 Q 2 R 3 N 4 B 5 P 6
int maxsc;
			...
			if (judge(dep, i, j))
			{
				...
				case 1:
					for (int k = 0; k < 8; k++)
					{
						...
						if (dep == 2)
						{
							if (sc + Map[plate[dep][tx][ty]] >= maxsc)
							{
								maxsc = sc + Map[plate[dep][tx][ty]];
								flag = 1;
							}
							continue;
						}
				...
				case 6:
					...
					else
					{
						if (i < 8)
						{
							if (plate[dep][i + 1][j] == 0)
							{
								if (sc + Map[plate[dep][i + 1][j]] >= maxsc)
								{
									maxsc = sc + Map[plate[dep][i + 1][j]];
									flag = 1;
								}
							}
							if (j > 1 && plate[dep][i + 1][j - 1] < 0)
							{
								if (sc + Map[plate[dep][i + 1][j - 1]] >= maxsc)
								{
									maxsc = sc + Map[plate[dep][i + 1][j - 1]];
									flag = 1;
								}
							}
							if (j < 8 && plate[dep][i + 1][j + 1] < 0)
